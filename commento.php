...
async function loadDrivers(){try{const response=await fetch('drivers.json',{cache:'no-store'});if(!response.ok)throw new Error('HTTP '+response.status);const raw=await response.json();driversData=Array.isArray(raw)?raw:(typeof raw==='string'?JSON.parse(raw):[]);if(!Array.isArray(driversData))driversData=[];console.log('Piloti caricati:',driversData.length);}catch(error){driversData=[];console.error('Errore nel caricamento dei piloti:',error);}}
