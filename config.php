...
// Secrets can be provided via environment variables or config/private.php (gitignored).
...
$secret = static function (string $key, string $env, string $default = '') use ($private): string {
    if (isset($private[$key]) && $private[$key] !== '') {
...
$geminiApiKey = !empty($savedSettings['gemini_api_key'])
    ? (string) $savedSettings['gemini_api_key']
    : $secret('gemini_api_key', 'GEMINI_API_KEY');
$geminiModelUrl = !empty($savedSettings['gemini_model_url'])
    ? (string) $savedSettings['gemini_model_url']
    : 'https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent';

return [
    'auth_user' => $secret('auth_user', 'SEO_AUTH_USER', 'admin'),
    'auth_password' => $secret('auth_password', 'SEO_AUTH_PASSWORD'),
    ...
    'media_dirs' => [
        '/home/urkv6v6p/domains/seo.formulapaddock.it/public_html/config/../immagini',
        '/home/urkv6v6p/domains/seo.formulapaddock.it/public_html/config/../uploads',
    ],
    'sitemaps' => [
    ...
    'sites' => [
        'formulapaddock' => [
            ...
            'username' => $secret('wp_formulapaddock_username', 'WP_FORMULAPADDOCK_USERNAME', 'admin'),
            'application_password' => $secret('wp_formulapaddock_application_password', 'WP_FORMULAPADDOCK_APPLICATION_PASSWORD'),
            'default_category' => '',
            'default_parent_page' => '',
        ],
        'wec' => [
            ...
            'username' => $secret('wp_wec_username', 'WP_WEC_USERNAME', 'admin'),
            'application_password' => $secret('wp_wec_application_password', 'WP_WEC_APPLICATION_PASSWORD'),
            'default_category' => '',
            'default_parent_page' => '',
        ],
        'formula2' => [
            ...
            'username' => $secret('wp_formula2_username', 'WP_FORMULA2_USERNAME', 'admin'),
            'application_password' => $secret('wp_formula2_application_password', 'WP_FORMULA2_APPLICATION_PASSWORD'),
            'default_category' => '',
            'default_parent_page' => '',
        ],
    ],
