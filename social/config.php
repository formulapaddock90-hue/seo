...
return [
    ...
    'buffer_share_mode'     => 'shareNow', // 'shareNow' per pubblicare subito, 'addToQueue' per la coda

    // ===================== LINKEDIN NATIVO =====================
    'linkedin_client_id'        => '',
    'linkedin_client_secret'    => '',
    'linkedin_oauth_token_json' => __DIR__ . '/credentials/linkedin-token.json',
    'linkedin_author_urn'       => '', // urn:li:person:ID oppure urn:li:organization:ID
    'linkedin_redirect_uri'     => (function () {
        if (!empty($_SERVER['HTTP_HOST'])) {
            $scheme = (strpos($_SERVER['HTTP_HOST'], 'localhost') === false) ? 'https' : 'http';
            $dir = rtrim(str_replace('\\', '/', dirname($_SERVER['SCRIPT_NAME'] ?? '/')), '/');
            return "{$scheme}://{$_SERVER['HTTP_HOST']}{$dir}/oauth_setup_linkedin.php";
        }
        return 'http://localhost/social/oauth_setup_linkedin.php';
    })(),
    ...
    'threads_redirect_uri'      => (function () {
        if (!empty($_SERVER['HTTP_HOST'])) {
            $scheme = (strpos($_SERVER['HTTP_HOST'], 'localhost') === false) ? 'https' : 'http';
            $dir = rtrim(str_replace('\\', '/', dirname($_SERVER['SCRIPT_NAME'] ?? '/')), '/');
            return "{$scheme}://{$_SERVER['HTTP_HOST']}{$dir}/oauth_setup_threads.php";
        }
        return 'http://localhost/social/oauth_setup_threads.php';
    })(),

    'ffmpeg_path' => (PHP_OS_FAMILY === 'Windows')
        ? __DIR__ . '/bin/ffmpeg-8.1.1-essentials_build/bin/ffmpeg.exe'
        : 'ffmpeg',
