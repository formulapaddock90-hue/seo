...
public sealed partial class TerminalInfoProvider
{
    ...
    private bool GetSixelSupported()
    {
        ...
        try
        {
            ...
            var trimmedResponse = Encoding.ASCII.GetString(buffer).TrimEnd((char)0);
            if (trimmedResponse.Length < 3)
            {
                _logger.LogDebug("Sixel support check: Response too short");
                return false;
            }

            var response = trimmedResponse[2..^1];
