...
public static partial class CommandHandler
{
    public static async Task Parse(string[] args, CancellationToken cancellationToken)
    {
        ...
        {
            Description = "Whether the API and graphical dashboard should be exposed at http://localhost:61937",
        };
        ...
        var sessionDirectoryArgument = new Argument<DirectoryInfo>("session-directory")
        {
            Description = "Path to the session directory containing live.jsonl and subscribe.json"
        };
        ...
        {
            Description =
                "Optional output file path. If not provided, defaults to final_standings_{timestamp}.txt in the session directory",
        };

        var exportCommand = new Command(
            "export",
            """
            Export final standings from a recorded session to a TXT file.
            """
        )
        {
            sessionDirectoryArgument,
            outputFileOption,
            dataDirectoryOption,
            isVerboseOption,
        };
        exportCommand.SetAction(res =>
            ExportSession(
                res.GetRequiredValue(sessionDirectoryArgument),
                res.GetValue(outputFileOption),
                res.GetValue(dataDirectoryOption),
                res.GetValue(isVerboseOption)
            )
        );
