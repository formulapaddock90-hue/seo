...
// ========== STANDING CSV FUNCTIONS ==========
async function loadStandingCsv() {
    const btn = document.getElementById('load-standing-csv');
    ...
    if (status) {
        ...
        status.textContent = '⏳ Caricamento dal CSV...';
        status.style.backgroundColor = '#e3f2fd';
    }

    try {
        const data = await apiGet('api/load-standing-csv.php');
        if (!data.success) {
            throw new Error(data.error || 'Errore sconosciuto');
        }
        ...
        state.sessionResultRows = rows;
        state.sessionResultMeta = {
            session_name: data.session_name || 'Classifica Standing',
            race_name: data.race_name || 'Da CSV',
            date: data.date || new Date().toLocaleDateString('it-IT'),
            source: 'CSV Standing'
        };
        ...
        if (tbody) {
            ...
            rows.forEach(row => {
                const tr = document.createElement('tr');
                tr.innerHTML = `<td>${row.position || '-'}</td><td>${row.number || '-'}</td><td>${escapeHtml(row.driver_name || '')}</td><td>${escapeHtml(row.team_name || '')}</td><td>${escapeHtml(row.best_lap || '') || '-'}</td><td>${escapeHtml(row.last_lap || '') || '-'}</td><td>${row.total_laps || '-'}</td><td>${row.gap || '-'}</td>`;
                tbody.appendChild(tr);
            });
        }

        btn.textContent = `✅ Caricati ${rows.length} piloti`;
        if (status) {
            status.textContent = `✅ CSV caricato (${rows.length} piloti)`;
            status.style.backgroundColor = '#c8e6c9';
        }

        setTimeout(() => {
            btn.disabled = false;
            btn.textContent = '📊 Carica CSV Standing';
        }, 2000);
