...
const FinalStandingsModule = {
    ...
    /**
     * Richiedi dati da UndercutF1 CSV (nuovo sistema)
     */
    async getStandingsFromExternalAPI() {
        try {
            // 1. Prova API import-undercut-standings (nuova, legge CSV da undercut)
            const response = await fetch('api/import-undercut-standings.php?action=fetch');

            if (response.ok) {
                const result = await response.json();
                if (result.ok && result.data && result.data.length > 0) {
                    console.log('✅ Classifica da UndercutF1 CSV:', result.data.length, 'piloti');
                    return result.data;
                }
            }

            console.warn('⚠️ Import UndercutF1 non disponibile, provo undercutf1-standings...');

            // 2. Fallback: prova vecchia API undercutf1-standings
            const fallbackResponse = await fetch('api/undercutf1-standings.php?action=fetch_standings');
            if (fallbackResponse.ok) {
                const result = await fallbackResponse.json();
                if (result.ok && result.data && result.data.length > 0) {
                    console.log('✅ Classifica da UndercutF1 API:', result.data.length, 'piloti');
                    return result.data;
                }
            }

            // 3. Fallback: dati mock
