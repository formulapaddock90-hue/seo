...
const postgara = {
    ...
    attachEventListeners() {
        ...
        const featuredFileInput = document.getElementById('postgara-featured-file-input');
        if (featuredFileInput) {
            featuredFileInput.addEventListener('change', async (e) => {
                const file = e.target.files?.[0];
                if (!file) return;
                this.featuredImageFile = file;
                this.featuredImageId = 0;
                // Anteprima locale immediata
                this.featuredImageUrl = URL.createObjectURL(file);
                const urlField = document.getElementById('postgara-featured-url-input');
                if (urlField) urlField.value = '';
                this.renderFeaturedImagePreview();
            });
        }

        const featuredUrlInput = document.getElementById('postgara-featured-url-input');
        if (featuredUrlInput) {
            featuredUrlInput.addEventListener('change', (e) => {
        ...
        const featuredClearBtn = document.getElementById('postgara-featured-clear-btn');
        if (featuredClearBtn) {
            featuredClearBtn.addEventListener('click', () => {
                ...
                const urlField = document.getElementById('postgara-featured-url-input');
                if (urlField) urlField.value = '';
                const fileField = document.getElementById('postgara-featured-file-input');
    ...
    async insertChartsIntoPost() {
        ...
        if (!reviewContent) {
            this.showError('❌ Modulo revisione non disponibile.');
            return;
        }

        const chartDefs = [
            { id: 'postgara-tyre-strategy-chart',         title: 'Strategia gomme' },
            { id: 'postgara-lap-times-g1-chart',          title: 'Tempi sul giro – Top 5' },
            { id: 'postgara-lap-times-g2-chart',          title: 'Tempi sul giro – Midfield' },
            { id: 'postgara-lap-times-g3-chart',          title: 'Tempi sul giro – Back' },
            { id: 'postgara-lap-times-g4-chart',          title: 'Tempi sul giro – Tail' },
            { id: 'postgara-speed-trap-chart',            title: 'Speed Trap' },
            { id: 'postgara-best-s1-chart',               title: 'Miglior Settore 1' },
            { id: 'postgara-best-s2-chart',               title: 'Miglior Settore 2' },
            { id: 'postgara-best-s3-chart',               title: 'Miglior Settore 3' },
            { id: 'postgara-compound-lap-count-chart',    title: 'Conteggio giri per compound' },
            { id: 'postgara-driver-compound-lap-count-chart', title: 'Giri per pilota/compound' },
        ];

        const htmlBlocks = [];
        chartDefs.forEach(def => {
            const ch = charts[def.id];
            if (!ch || typeof ch.data !== 'object' || typeof ch.options !== 'object') {
                console.warn(`Chart ${def.id} not found or invalid`);
                return;
            }

            const chartData = ch.data;
            const chartOptions = ch.options;
            const chartType = ch.config?.type || 'bar';
            ...
            const chartConfig = {
                ...
                data: {
                    ...
                    datasets: (chartData.datasets || []).map(ds => ({
                        ...
                        fill: ds.fill !== undefined ? ds.fill : undefined,
                        tension: ds.tension || undefined,
                        pointRadius: ds.pointRadius || undefined,
                        spanGaps: ds.spanGaps !== undefined ? ds.spanGaps : undefined,
                        indexAxis: ds.indexAxis || undefined,
                    }))
                },
                options: {
                    responsive: true,
            ...
            const rawHtmlBlock = `<div style="position:relative;width:100%;margin:20px 0;">
  <canvas id="${def.id}-wp"></canvas>
</div>
<script>
(function() {
  function renderChart() {
    const canvas = document.getElementById('${def.id}-wp');
    if (!canvas) return;
  ...
  if (document.readyState === 'loading') {
    ...
    renderChart();
            ...
            htmlBlocks.push(rawHtmlBlock);
            console.log(`✓ Generated HTML block: ${def.title}`);
        });
        ...
        try {
            ...
            chartDefs.forEach(def => {
                ...
                const dataJson = JSON.stringify(chartConfig).replace(/"/g, '&quot;');
                appendHtml += `[grafico-postgara title="${def.title}" data="${dataJson}"]\n\n`;
            });

            reviewContent.value = existingContent + appendHtml;
            reviewContent.dispatchEvent(new Event('input'));

            this.setEditorStatus(`✅ ${htmlBlocks.length} grafici inseriti nell'articolo.`);
            this.showError(`✅ ${htmlBlocks.length} shortcode grafici inseriti.`);
    ...
    renderFeaturedImagePreview() {
        const preview = document.getElementById('postgara-featured-preview');
        const urlInput = document.getElementById('postgara-featured-url-input');
        if (!preview) return;
        if (this.featuredImageUrl) {
            preview.innerHTML = `<img src="${this.featuredImageUrl}" alt="In evidenza" style="max-height:56px;max-width:100px;border-radius:4px;object-fit:cover;vertical-align:middle;">`;
            if (urlInput && !urlInput.value && !this.featuredImageFile) urlInput.value = this.featuredImageUrl;
        } else {
    ...
    async uploadFeaturedImage(file) {
        const form = new FormData();
        form.append('team', '__featured__');
        form.append('image', file);
        const res = await fetch('api/postgara-data.php', { method: 'POST', body: form });
        const data = await res.json().catch(() => ({}));
        if (!res.ok || !data.ok) throw new Error(data.message || 'Errore upload immagine in evidenza');
        return data.image || '';
    },

    async syncFeaturedImageToWordPress() {
        if (!this.featuredImageUrl || this.featuredImageId > 0) return;
        const site = document.getElementById('wp-site')?.value;
        if (!site) return;
        try {
            const res = await apiPost('api/wordpress.php?action=upload_media_batch', { site, paths: [this.featuredImageUrl] });
            const uploaded = (res?.uploads || {})[this.featuredImageUrl];
            if (uploaded?.id) {
                this.featuredImageId = parseInt(uploaded.id, 10) || 0;
                if (uploaded.url) this.featuredImageUrl = uploaded.url;
            }
        } catch { /* non bloccante */ }
    },
    ...
    async generateSeoArticle() {
        ...
        try {
            ...
            // Upload file featured se non ancora sul server
            if (this.featuredImageFile && !this.featuredImageUrl.startsWith('http')) {
                try {
                    const serverUrl = await this.uploadFeaturedImage(this.featuredImageFile);
                    this.featuredImageUrl = serverUrl;
                    this.featuredImageFile = null;
                    this.renderFeaturedImagePreview();
                } catch (e) { console.warn('Featured image upload failed:', e); }
            }

            // Sincronizza featured image su WordPress
            await this.syncFeaturedImageToWordPress();
            ...
            setTimeout(async () => {
                const res = await apiPost('api/wordpress.php?action=generate_postgara_article', {
                    teams: payload,
                    featured_image_id:  this.featuredImageId  || 0,
